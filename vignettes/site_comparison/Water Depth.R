...
setwd("..//Input/")
write.table(hidrologia.bromelia, "hidrologia da bromélia.xls", sep="\t", row.names = FALSE)
...
model4a<-lmer(pc1 ~ volume+chuva+(1|bloco)+volplanta, data=dados)
model4a@theta
qqnorm(resid(model4a, type="deviance"));qqline(resid(model4a, type="deviance"))

model4b<-lmer(pc1 ~ volume+(1|bloco), data=dados)
model4b@theta
...
model4d<-lmer(pc1 ~ volume+scale(volplanta)+(scale(volplanta)|bloco), data=dados)
model4d@theta
qqnorm(resid(model4d, type="deviance"));qqline(resid(model4d, type="deviance"))

anova(model4, model4a, model4b, model4c, model4d)

summary(model4d)
Anova(model4d, type="II", test.statistic = "F", error.estimate = "deviance")
